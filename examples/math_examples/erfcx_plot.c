...
int main(void)
{
    ...
    for (y=0; y<size; ++y)
    {
        ...
        for (x=0; x<size; ++x)
        {
            ...
            erfcx_Px = rssringoccs_Erfcx_Double(Px);

            if ((rssringoccs_Abs_Double(Px) < pixel_width*y_to_x_scale) ||
                (rssringoccs_Abs_Double(Py - erfcx_Px) < pixel_width) ||
                (rssringoccs_Abs_Double(Py) < pixel_width))
