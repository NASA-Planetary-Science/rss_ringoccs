...
;------------------------------------------------------------------------------;
;   Purpose:                                                                   ;
;       Computes the complex-valued Fresnel cosine.                            ;
;   Arguments:                                                                 ;
;       VAR (real, complex, array-like):                                       ;
;           The independent variable.                                          ;
;   Output:                                                                    ;
;       F_COS (real, complex, array-like):                                     ;
;           The Fresnel cosine of the input.                                   ;
;------------------------------------------------------------------------------;
;   Author: Ryan Maguire                                                       ;
;   Date:   2017                                                               ;
;------------------------------------------------------------------------------;

; Computes the Fresnel cosine of the input.
FUNCTION FRESNEL_COS, VAR
    ...
    ; The Fresnel cosine is a sum of two error functions. Compute these.
    LEFT_TERM = (SOUTH_EAST / 4.0) * ERF(NORTH_EAST * SCALE_FACTOR)
    RIGHT_TERM = (NORTH_EAST / 4.0) * ERF(SOUTH_EAST * SCALE_FACTOR)

    F_COS = LEFT_TERM + RIGHT_TERM

    ; If the input is real, we should return the output as a real variable.
    IF IS_REAL THEN BEGIN
        RETURN, REAL_PART(F_COS)
    ENDIF
