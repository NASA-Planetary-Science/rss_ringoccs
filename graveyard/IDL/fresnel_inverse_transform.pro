...
;------------------------------------------------------------------------------;
;   Purpose:                                                                   ;
;       Computes the inverse Fresnel transform of complex-valued data.         ;
;   Arguments:                                                                 ;
...
;   Output:                                                                    ;
;       T (complex):                                                           ;
;           The Fresnel inversion of the input T_HAT.                          ;
;   References:                                                                ;
...
;   Date:   2018/04/12                                                         ;
;------------------------------------------------------------------------------;
FUNCTION FRESNEL_INVERSE_TRANSFORM, T_HAT, KER, DX, F_SCALE
    ...
    IF (CHECK_REAL(T_HAT) EQ 0) AND (CHECK_COMPLEX(T_HAT) EQ 0) THEN BEGIN
        MESSAGE, "T_HAT must be real or complex valued."
    ENDIF
    ...
    IF N_ELEMENTS(T_HAT) NE N_ELEMENTS(KER) THEN BEGIN
        MESSAGE, "T_HAT and KER have a different number of elements."
    ENDIF
