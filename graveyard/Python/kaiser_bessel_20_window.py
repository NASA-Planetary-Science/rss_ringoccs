...
################################################################################
#   Purpose:                                                                   #
#       Computes the Kaiser Bessel window function with alpha = 2 pi.          #
################################################################################
#                                 DEPENDENCIES                                 #
################################################################################
#   1.) numpy                                                                  #
...
def kb20(w_in, bin_width, check_error = True):
    """
        Function:
            kb20
        Purpose:
            Create the Kaiser-Bessel window with alpha = 2 pi.
        Arguments:
            w_in (float):
                Window width.
            bin_width (float):
        ...
        Outputs:
            w_func (numpy.ndarray):
                The Kaiser-Bessel window with alpha = 2 pi.
        Dependencies:
            [1] numpy
            [2] scipy
        Notes:
        ...
        Warnings:
            [1] None of the Kaiser-Bessel windows evaluate to zero at
                ...
                discontinuity in the window. For alpha = 2.0 * pi,
                the endpoint is 0.01147994, which is small.
        References:
    ...
    # 2 pi, the parameter for the Kaiser-Bessel window.
    alpha = 6.283185307179586

    # 1 / I0(2 pi) where I0 is the zeroth modified Bessel function.
    factor = 0.011479934537958647
