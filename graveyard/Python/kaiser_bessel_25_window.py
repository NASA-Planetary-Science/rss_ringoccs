...
################################################################################
#   Purpose:                                                                   #
#       Computes the Kaiser Bessel window function with alpha = 2.5 pi.        #
################################################################################
#                                 DEPENDENCIES                                 #
################################################################################
#   1.) numpy                                                                  #
...
def kb25(w_in, bin_width, check_error = True):
    """
        Function:
            kb25
        Purpose:
            Create the Kaiser-Bessel window with alpha = 2.5 pi.
        Arguments:
            w_in (float):
                Window width.
            bin_width (float):
        ...
        Outputs:
            w_func (numpy.ndarray):
                The Kaiser-Bessel window with alpha = 2.5 pi.
        Dependencies:
            [1] numpy
            [2] scipy
        Notes:
        ...
        Warnings:
            [1] None of the Kaiser-Bessel windows evaluate to zero at
                ...
                the endpoint is 0.00268, which is small.
        References:
    ...
    # 2.5 pi, the parameter for the Kaiser-Bessel window.
    alpha = 7.853981633974483

    # 1 / I0(2.5 pi) where I0 is the zeroth modified Bessel function.
    factor = 2.6808168640244157E-03
