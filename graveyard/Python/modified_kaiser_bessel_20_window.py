...
def kbmd20(w_in, bin_width, check_error = True):
    """
        Function:
            kbmd20
        Purpose:
            Create the Modified Kaiser-Bessel window with alpha = 2 pi.
        Arguments:
            w_in (float):
                Window width.
            bin_width (float):
        ...
        Outputs:
            w_func (numpy.ndarray):
                The modified Kaiser-Bessel window with alpha = 2 pi.
        Dependencies:
            [1] numpy
            [2] scipy
        Notes:
            ...
            [2] We automatically multiply the alpha parameter by pi,
                so the kbmd20 window function has an alpha value of
                alpha = 2.0 * pi
    ...
    # 1 / (I0(2 pi) - 1) where I0 is the zeroth modified Bessel function.
    factor = 1.1613253932880810E-02
