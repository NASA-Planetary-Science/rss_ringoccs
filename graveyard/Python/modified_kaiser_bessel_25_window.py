...
def kbmd25(w_in, bin_width, check_error = True):
    """
        Function:
            kbmd25
        Purpose:
            Create the Modified Kaiser-Bessel window with alpha = 2.5 pi.
        Arguments:
            w_in (float):
                Window width.
            bin_width (float):
        ...
        Outputs:
            w_func (numpy.ndarray):
                The modified Kaiser-Bessel window with alpha = 2.5 pi.
        Dependencies:
            [1] numpy
            [2] scipy
        Notes:
            ...
            [2] We automatically multiply the alpha parameter by pi,
                so the kbmd25 window function has an alpha value of
                alpha = 2.5 * pi
    ...
    # 1 / (I0(2.5 pi) - 1) where I0 is the zeroth modified Bessel function.
    factor = 2.6880229613099804E-03
