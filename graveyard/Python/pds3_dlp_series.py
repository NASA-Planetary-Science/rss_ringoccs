...
def write_dlp_series_data(dlp_inst, fmt, out_file):
    ...
    #   as an attribute
    tau_norm_vals = -numpy.sin(abs(dlp_inst.B_rad_vals)) * numpy.log(
        ...
        for n in range(npts):
            f.write(
                format_str % (
                    ...
                    numpy.degrees(dlp_inst.phi_rl_rad_vals[n]),
                    numpy.degrees(dlp_inst.phi_rad_vals[n]),
                    tau_norm_vals[n],
                    numpy.degrees(dlp_inst.phase_rad_vals[n]),
                    dlp_inst.tau_threshold_vals[n],
                    dlp_inst.t_oet_spm_vals[n],
                    dlp_inst.t_ret_spm_vals[n],
                    dlp_inst.t_set_spm_vals[n],
                    numpy.degrees(dlp_inst.B_rad_vals[n]),
                    '\r\n'
                )
        )
...
def get_dlp_series_info(rev_info, dlp_inst, series_name, prof_dir):
    ...
    MINIMUM_RING_LONGITUDE = str(min(numpy.degrees(dlp_inst.phi_rl_rad_vals)))
    MAXIMUM_RING_LONGITUDE = str(max(numpy.degrees(dlp_inst.phi_rl_rad_vals)))
    MINIMUM_OBSERVED_RING_AZIMUTH = str(min(numpy.degrees(dlp_inst.phi_rad_vals)))
    MAXIMUM_OBSERVED_RING_AZIMUTH = str(max(numpy.degrees(dlp_inst.phi_rad_vals)))
    MINIMUM_OBSERVED_RING_ELEVATION = str(min(numpy.degrees(dlp_inst.B_rad_vals)))
    MAXIMUM_OBSERVED_RING_ELEVATION = str(max(numpy.degrees(dlp_inst.B_rad_vals)))
    ...
    SERIES_DESCRIPTION = (
        '"This series contains variations of the' + sd +
    ...
    reference_times = [
        ...
        OBJECT_REFERENCE_TIME, OBJECT_REFERENCE_TIME, OBJECT_REFERENCE_TIME, es
    ]
...
def write_dlp_series(rev_info, dlp_inst, title, outdir, prof_dir):
    """
    This function writes a DLP series,
    which includes a data and label file.
    Args:
        rev_info (dict):
            Dictionary with keys: rsr_file, band, year, doy, dsn
            occ_dir, planetary_occ_flag, rev_num
        dlp_inst (class):
            Instance of NormDiff class
        title (str):
            Name of the output .TAB and .LBL file, not including
            extensions. Date in YYYYMMDD format will be added
            onto series_name
        outdir (str):
            Path to output directory
        prof_dir (str):
            Direction of ring occultation for this dlp_inst
    Notes:
