...
################################################################################
#   Purpose:                                                                   #
#       Functions for getting PDS3 labels.                                     #
################################################################################
#   Author: Jolene Fong                                                        #
#   Date:   2018/07/27                                                         #
################################################################################
        ...
        for i in range(nkern):
            ...
            if i == 0:
                if nkern == 1:
                    ...
                    app = ','
                f.write(
                    kernel_kwd.ljust(npad0) + eq + kernel_val_list[i] + app + cr
                )
    ...
    for skey in series_key_order:
        ...
        if this_key == 'OBJECT':
            ...
            for d in range(nlines):
                if d==0:
                    f.write(indent + this_key.ljust(npad1) + eq + lines[d] + cr)
                else:
            ...
            if (this_kwd == 'Input Variables') or (this_kwd == 'Input Keywords'):
                ...
                for varkey in this_val:
                    ...
                    if isinstance(this_input, dict):
                        ...
                        if varkey == 'kernels':
                            this_input = [x.split('/')[-1] for x in this_input]
                        if varkey == 'rsr_file':
