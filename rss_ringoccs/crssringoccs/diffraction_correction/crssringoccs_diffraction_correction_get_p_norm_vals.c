...
crssringoccs_DiffractionCorrection_Get_P_Norm_Vals(PyObject *op, void *closure)
{
    ...
    /*  Buffer for the diffracted power data.                                 */
    double *diffracted_power = NULL;
    ...
    /*  Otherwise, compute the diffracted power from the T_in array.          */
    diffracted_power = TMPL_MALLOC(double, self->tau->n_used);
    ...
    for (n = 0; n < self->tau->n_used; ++n)
    {
        ...
        diffracted_power[n] = tmpl_CDouble_Abs_Squared(transmittance);
    }

    /*  Wrap the data in a numpy array for ease of use.                       */
    crssringoccs_Create_Real_Numpy_Array(
        &self->p_norm_vals,             /*  The Python object.                */
        diffracted_power,               /*  The real-valued C data.           */
...
/*  End of crssringoccs_DiffractionCorrection_Get_P_Norm_Vals.                */
