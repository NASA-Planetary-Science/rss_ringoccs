                                                      ...
                                                      void *closure)
{
    ...
    /*  Buffer for the reconstructed phase data.                              */
    double *phase = NULL;
    ...
    /*  The real-valued phase_deg_vals array is computed from the complex     *
     *  T_out array. If T_out is NULL, set phase_deg_vals to None.            */
    if (!self->tau->T_out)
    {
        PyObject *tmp = self->phase_deg_vals;
        Py_INCREF(Py_None);
        self->phase_deg_vals = Py_None;
        Py_CLEAR(tmp);
    ...
    /*  Otherwise, compute the reconstructed phase from the T_out array.      */
    phase = TMPL_MALLOC(double, self->tau->n_used);
    ...
    for (n = 0; n < self->tau->n_used; ++n)
    {
         ...
         *  is given by the argument of T_out. Compute this.                  */
        const double phase_rad = tmpl_CDouble_Argument(transmittance);

        /*  Convert from radians to degrees.                                  */
        phase[n] = phase_rad * tmpl_double_rad_to_deg;
    }

    /*  Wrap the data in a numpy array for ease of use.                       */
    crssringoccs_Create_Real_Numpy_Array(
        &self->phase_deg_vals,          /*  The Python object.                */
        phase,                          /*  The real-valued C data.           */
...
/*  End of crssringoccs_DiffractionCorrection_Get_Phase_Deg_Vals.             */
