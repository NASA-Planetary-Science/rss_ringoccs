...
crssringoccs_DiffractionCorrection_Get_Power_Vals(PyObject *op, void *closure)
{
    ...
    /*  Buffer for the reconstructed power data.                              */
    double *power = NULL;
    ...
    /*  The real-valued power_vals array is computed from the complex T_out   *
    ...
    /*  Otherwise, compute the reconstructed power from the T_out array.      */
    power = TMPL_MALLOC(double, self->tau->n_used);
    ...
    for (n = 0; n < self->tau->n_used; ++n)
    {
        ...
        power[n] = tmpl_CDouble_Abs_Squared(transmittance);
    }

    /*  Wrap the data in a numpy array for ease of use.                       */
    crssringoccs_Create_Real_Numpy_Array(
        &self->power_vals,             /*  The Python object.                 */
        power,                          /*  The real-valued C data.           */
...
/*  End of crssringoccs_DiffractionCorrection_Get_Power_Vals.                 */
