                                                    ...
                                                    void *closure)
{
    ...
    /*  Buffer for the forward model optical depth data.                      */
    double *model_optical_depth = NULL;
    ...
    /*  The real-valued tau_fwd_vals array is computed from the complex T_fwd *
    ...
    /*  Otherwise, compute the model optical depth from the T_fwd array.      */
    model_optical_depth = TMPL_MALLOC(double, self->tau->n_used);

    /*  Check to make sure malloc did not fail.                               */
    if (!model_optical_depth)
    ...
    for (n = 0; n < self->tau->n_used; ++n)
    {
        ...
        model_optical_depth[n] = -mu * tmpl_Double_Log(power);
    }

    /*  Wrap the data in a numpy array for ease of use.                       */
    crssringoccs_Create_Real_Numpy_Array(
        &self->tau_fwd_vals,            /*  The Python object.                */
        model_optical_depth,            /*  The real-valued C data.           */
...
/*  End of crssringoccs_DiffractionCorrection_Get_Tau_Fwd_Vals.               */
