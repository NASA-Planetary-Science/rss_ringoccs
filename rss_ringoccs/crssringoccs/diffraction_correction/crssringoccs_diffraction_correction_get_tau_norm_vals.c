                                                     ...
                                                     void *closure)
{
    ...
    /*  Buffer for the diffracted optical depth data.                         */
    double *diffracted_optical_depth = NULL;
    ...
    /*  The real-valued tau_norm_vals array is computed from the complex T_in *
    ...
    /*  Otherwise, compute the diffracted optical depth from the T_in array.  */
    diffracted_optical_depth = TMPL_MALLOC(double, self->tau->n_used);

    /*  Check to make sure malloc did not fail.                               */
    if (!diffracted_optical_depth)
    ...
    for (n = 0; n < self->tau->n_used; ++n)
    {
         ...
         *  range for processing. Shift the index for T_in and grab the data. */
        const size_t index = self->tau->start + n;
        const tmpl_ComplexDouble transmittance = self->tau->T_in[index];
        ...
        diffracted_optical_depth[n] = -mu * tmpl_Double_Log(power);
    }

    /*  Wrap the data in a numpy array for ease of use.                       */
    crssringoccs_Create_Real_Numpy_Array(
        &self->tau_norm_vals,           /*  The Python object.                */
        diffracted_optical_depth,       /*  The real-valued C data.           */
    ...
    if (!self->tau_norm_vals)
    {
        ...
        TMPL_FREE(diffracted_optical_depth);
        return NULL;
    }
...
/*  End of crssringoccs_DiffractionCorrection_Get_Tau_Norm_Vals.              */
