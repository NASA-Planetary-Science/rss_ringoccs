...
del dcfile
from .diffcorr import diffraction_correction
...
class compare_tau(object):
    def __init__(self,geo,cal,dlp,tau,res,occ=False,rng='all',
        wtype="kb25",bfac=True,fft=False,verbose=True,norm=True):

        data        = extract_csv_data(geo,cal,dlp,occ=occ,
            taudata=tau,verbose=verbose)
        tr          = data.tau_rho
