...
def get_planet_occ_times(et_vals, obs, planet, spacecraft, height_above=500.,
    ...
    Args:
        ...
        obs (str):              Observer name -- must be compatible with NAIF.
        planet (str):           Planet name -- must be compatible with NAIF
        spacecraft (str):       Spacecraft name -- must be compatible with NAIF
        height_above (float64): Height in km to be added to planet radius to
                                account for the atmosphere. Default is 500km.
        kernels (list):         List of NAIF kernels, including path.

    Output:
        et_blocked_vals (np.ndarray): Array of observed event times in ET sec
                                      at which spacecraft-to-obs ray is blocked
                                      by the planet.
