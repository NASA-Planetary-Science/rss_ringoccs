...
{                                                                              \
    ...
    type dx, y, y_prime;                                                       \
    unsigned int n, max_iter;                                                  \
                                                                               \
    /*  Set the max iteration to 20, which should be more than enough for    */\
    /*  any reasonable setting. The number of decimals of precision doubles  */\
    /*  with every iteration of Newton-Raphson, so if convergence doesn't    */\
    /*  happen after a few iterations it probably won't converge.            */\
    max_iter = 20;                                                             \
                                                                               \
...
)
{
    ...
    /*  If the derivative is zero at your initial guess, Newton-Raphson       *
     *  fails. Return Not-a-Number in this case.                              */
    if (rssringoccs_Complex_Compare(w_prime, rssringoccs_Complex_Zero))
        return rssringoccs_Complex_NaN;

    /*  Compute the first iteration of Newton-Raphson.                        */
    dz = rssringoccs_Complex_Divide(w, w_prime);
