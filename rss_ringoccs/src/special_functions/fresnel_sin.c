...
float rssringoccs_Fresnel_Sin_Float(float x)
{
    ...
    else if (arg < 1.0e16)
    {
        ...
        sx = sx + ((x > 0) - (x < 0))*SQRT_PI_BY_8;
    }

    /*  For large values, return the limit of S(x) as x -> +/- infinity.      */
    else
        sx = ((x > 0) - (x < 0))*SQRT_PI_BY_8;

    return sx;
}

double rssringoccs_Fresnel_Sin_Double(double x)
{
    /* Variables for S(x) and powers of x, respectively. */
    double sx;
...
long double rssringoccs_Fresnel_Sin_LongDouble(long double x)
{
    /* Variables for S(x) and powers of x, respectively. */
    long double sx;
    long double arg = x*x;
