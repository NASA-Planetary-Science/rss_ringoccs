...
class ExtractCSVData(object):
    """
        ...
        Attributes:
            ...
            :phase_vals:
                ...
                a NoneType variable. Units are in degrees.
            :phi_deg_vals:
                The observed ring azimuth angle, in degrees.
            :phi_rl_deg_vals:
    ...
    def __init__(self, geo, cal, dlp, tau=None, verbose=True, use_deprecate=False):
        ...
        try:
            ...
            if verbose:
                print("\t self.D_km_vals:",self.D_km_vals)
                print("\t self.rho_dot_kms_vals:",self.rho_dot_kms_vals)
        ...
        self.f_sky_hz_vals = np.interp(self.t_oet_spm_vals, cal_spm,
                                       self.f_sky_hz_vals)
        if verbose:
            print("\tPrior to interpolation, self.D_km_vals",self.D_km_vals)
            print("\tPrior to interpolation, self.rho_km_vals",self.rho_km_vals)
            print("\tPrior to interpolation, geo_rho",geo_rho)

        x  = self.t_oet_spm_vals
        xp = np.copy(cal_spm)
        fp = np.copy(self.D_km_vals)
        if np.all(np.diff(xp) > 0) == False:
            if verbose:
                print("\tReversing arrays")
            xp = xp[::-1]
            fp = fp[::-1]
            if np.all(np.diff(xp) > 0) == False:
                print("*** TROUBLE! Even after reversing array, still not in monotonic order!")
            if verbose:
                print("\txp = geo_rho = ",xp)
                print("\tfp = self.D_km_vals = ",fp)
        # self.D_km_vals = np.interp(self.rho_km_vals, xp, fp)
        self.D_km_vals = np.interp(x, xp, fp)
        if verbose:
            print("\tAfter interpolation, self.D_km_vals",self.D_km_vals)

        if verbose:
            print("\tPrior to interpolation, self.rho_dot_kms_vals",self.rho_dot_kms_vals)
        fp = np.copy(self.rho_dot_kms_vals)
#        self.rho_dot_kms_vals = np.interp(self.rho_km_vals, geo_rho,
#                                          self.rho_dot_kms_vals)
        self.rho_dot_kms_vals = np.interp(x, xp, fp)
        if verbose:
            print("\tAfter interpolation, self.rho_dot_kms_vals",self.rho_dot_kms_vals)
        ...
        self.rx_km_vals = np.interp(x,xp, rx_geo)
        self.ry_km_vals = np.interp(x,xp, ry_geo)
        self.rz_km_vals = np.interp(x,xp, rz_geo)
        if verbose:
            print("\tAfter interpolation, self.rz_km_vals",self.rz_km_vals)
...
class GetUranusData(object):
    def __init__(self, geodata, dlpdata, tau=None, verbose=False, occ="E"):
        ...
        try:
            ...
            self.rho_km_vals = np.array(dlp_dat.rho_km_vals)
            self.raw_tau_vals = np.array(dlp_dat.raw_tau_vals)
            self.phase_deg_vals = np.degrees(np.array(dlp_dat.phase_rad_vals))
            self.phi_deg_vals = np.degrees(np.array(dlp_dat.phi_rad_vals))
            self.B_deg_vals = np.degrees(np.array(dlp_dat.B_rad_vals))
            self.t_oet_spm_vals = np.array(dlp_dat.t_oet_spm_vals)
            self.t_ret_spm_vals = np.array(dlp_dat.t_ret_spm_vals)
            self.t_set_spm_vals = np.array(dlp_dat.t_set_spm_vals)
            self.rho_corr_pole_km_vals = np.array(dlp_dat.rho_km_vals)
            self.rho_corr_timing_km_vals = np.array(dlp_dat.rho_km_vals)
            self.phi_rl_deg_vals = np.degrees(np.array(dlp_dat.phi_rl_rad_vals))
            self.raw_tau_threshold_vals = np.array(dlp_dat.raw_tau_threshold_vals)
            ...
            raise TypeError(
                """
                    \r\tError Encountered: rss_ringoccs
                    \r\t\t%s\n
                    \r\tCould not convert variable into a numpy array.
                    \r\tCheck input GEO, CAL, and DLP files.
                """ % (fname, errmess)
            )
        ...
        if (np.min(drdt) < 0.0) and (np.max (drdt) > 0.0):
           raise ValueError(
            ...
            # The rev is ingress, so flip GEO variables.
# try turning this off - it is not essential
            # self.rho_dot_kms_vals = np.abs(self.rho_dot_kms_vals[::-1])
# This line is the offending one it must be turned off
            #self.D_km_vals = self.D_km_vals[::-1]
# this line is essential
            geo_rho = geo_rho[::-1]
# added these 2023 Aug 8 - they are essential, too
            rx_geo = rx_geo[::-1]
            ry_geo = ry_geo[::-1]
            rz_geo = rz_geo[::-1]
        elif (drdt > 0.0).all():
...
class GetUranusDataMerged(object):
    def __init__(self, merged_csvfile,verbose=False):
        if verbose:
            print("\tRetrieving Variables...")
# create dict with all necessary values from single CSV file

        import numpy as np
        with open(merged_csvfile,'r') as f:
            line = f.readline()
        keys = [x.strip() for x in line.split(',')]
        array = np.loadtxt(merged_csvfile,skiprows=1,delimiter=',',unpack=True)
        d={}
        for i,key in enumerate(keys):
            d[key]=array[i,:]

        self.rho_km_vals = np.array(d['rho_km_vals'])
        self.raw_tau_vals = np.array(d['raw_tau_vals'])
        self.phase_deg_vals = np.array(d['phase_deg_vals'])
        # NOTE NEEDED? self.phi_ora_deg_vals = np.array(d['phi_ora_deg_vals'])
        self.B_deg_vals = np.array(d['B_deg_vals'])
        self.t_oet_spm_vals = np.array(d['t_oet_spm_vals'])
        self.t_ret_spm_vals = np.array(d['t_ret_spm_vals'])
        self.t_set_spm_vals = np.array(d['t_set_spm_vals'])
        self.rho_corr_pole_km_vals = np.array(d['rho_corr_pole_km_vals'])
        self.rho_corr_timing_km_vals = np.array(d['rho_corr_timing_km_vals'])
        self.phi_rl_deg_vals = np.array(d['phi_rl_deg_vals'])
        self.phi_deg_vals = np.array(d['phi_deg_vals'])

        self.raw_tau_threshold_vals = np.array(d['raw_tau_threshold_vals'])
        self.p_norm_vals = np.array(d['p_norm_vals'])
        self.D_km_vals = np.array(d['D_km_vals'])
        self.f_sky_hz_vals = np.array(d['f_sky_hz_vals'])
        self.rho_dot_kms_vals = np.array(d['rho_dot_kms_vals'])

        self.rx_km_vals = np.array(d['rx_km_vals'])
        self.ry_km_vals = np.array(d['ry_km_vals'])
        self.rz_km_vals = np.array(d['rz_km_vals'])
        self.tau_rho = None
        self.tau_vals = None
        self.phase_vals = None
        self.power_vals = None
