...
Revisions:
      gjs_freq_offset.py
   Feb 20 2018 - gsteranka - Original version
   Feb 21 2018 - gsteranka - Added evaluation of f_sky_pred at the "f_SPM"
                             in here
      gjs_freq_offset_v2.py
   Mar 02 2018 - gsterankas - Original version. Edited from past version to
...
def calc_freq_offset(spm_vals, IQ_m, dt_freq=8.192, spm_range=None, TEST=False):
    ...
    Example:
        ...
        >>> (f_spm, f_offset) = calc_freq_offset(spm_vals, IQ_m, \
...
def __find_peak_freq(spm,IQ_m, cont_fft_freq_range):
    ...
    # Hamming window
    weight = 0.5*(1.0 - np.cos(2.0*np.pi*np.arange(pts_per_fft)/(pts_per_fft-1)))
