                                           ...
                                           double ry, double rz)
{
    ...
    rho = r;
    dphi = rssringoccs_Fresnel_dPsi_dPhi_Ellipse(k, rho, r0, phi, phi0,
                                                 B, D, ecc, peri);

    while(fabs(dphi) > EPS)
    {
        dphi = rssringoccs_Fresnel_dPsi_dPhi_Ellipse(k, rho, r0, phi, phi0, B,
                                                      D, ecc, peri);
        phi -= dphi/rssringoccs_Fresnel_d2Psi_dPhi2(k, rho, r0, phi, 
                                                    phi0, B, D);
        ++i;
        if (i > toler)
            break;
