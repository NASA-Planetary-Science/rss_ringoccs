...
)
{
    ...
    tmpl_ComplexDouble left, mid, right;
    tmpl_ComplexDouble integrand, left_midpoint, right_midpoint;
    ...
    for (n = 0; n < nw_pts - 2; n += 2)
    {
        ...
        left_midpoint = tmpl_CDouble_Midpoint(left, mid);
        right_midpoint = tmpl_CDouble_Midpoint(mid, right);

        integrand = tmpl_CDouble_Filon02_Integrand(
            left_midpoint,
            right_midpoint,
            left_psi,
            mid_psi,
...
/*  End of rssringoccs_Fresnel_Transform_Newton_Filon02.                      */
