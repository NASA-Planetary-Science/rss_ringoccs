...
)
{
    ...
    tmpl_TwoVectorDouble rho0 = tmpl_2DDouble_Polard(
        tau->rho_km_vals[offset], tau->phi_deg_vals[offset]
    );
    ...
    tmpl_ThreeVectorDouble R = tmpl_3DDouble_Rect(
        tau->rx_km_vals[offset],
        tau->ry_km_vals[offset],
        tau->rz_km_vals[offset]
    );
    ...
    {
        rho0 = tmpl_2DDouble_Polard(
            tau->rho_km_vals[offset + 1], tau->phi_deg_vals[offset + 1]
        );

        rho = tmpl_2DDouble_Polard(
            tau->rho_km_vals[center], tau->phi_deg_vals[offset + 1]
        );

        R = tmpl_3DDouble_Rect(
            tau->rx_km_vals[offset + 1],
            tau->ry_km_vals[offset + 1],
            tau->rz_km_vals[offset + 1]
        );

        next_psi = -tmpl_Double_Stationary_Cyl_Fresnel_Psi(
            tau->k_vals[offset + 1], &rho, &rho0, &R, tau->EPS, tau->toler
        );
