...
void rssringoccs_Diffraction_Correction_SimpleFFT(rssringoccs_TAUObj *tau)
{
    ...
    /*  New scope so that we may declare things constant, which is desirable  *
     *  for parallel processing. C89 does not permit mixing declarations with *
     *  code and we needed to perform error checks above. Introducing a new   *
     *  scope keeps things portable.                                          */
    {
        /*  Variable for the center of the data set. The geometry is computed *
         *  with respect to this point.                                       */
        const size_t center = tau->start + tau->n_used / 2;
         ...
         *  region the tapering function is zero and kills off the Fresnel    *
         *  kernel, so there is no point in computing it.                     */
        const double w_thresh = 0.5 * tau->w_km_vals[center];

        /*  The amount of data that the FFTs process is given by the total    *
         *  number of points in the data, plus half-windows to either side.   */
        const size_t data_size = tau->n_used + nw_pts - 1;
        const size_t arr_size = next_pow_2(2 * data_size - 1);

        /*  Shift factor for converting the FFT data back into natural order. */
        const size_t shift = data_size >> 1;

        /*  The integral needs half a window to the left of the starting      *
         *  point, and half a window to the right of the end point.           *
         *  Shift the start back.                                             */
        const size_t start = center - shift;
        ...
        if (!ker || !T_in)
        {
            ...
            /*  ker and T_in are either NULL or point to valid data allocated *
        ...
        *  variables. This saves us calls to malloc and wasteful memory use.  */
         ...
         *  The Tau object already has T, compute the kernel across the data  *
         *  set. Note that we are using the geometry for the point in the     *
         *  center of the data set and assuming this approximates the         *
         *  geometry for the entire region being processed. Because of this,  *
         *  trying to process a large region will result in a very poor       *
         *  reconstruction.                                                   */
#ifdef _OPENMP
#pragma omp for schedule(static)
        ...
        *  Compute the FFTs of the complex diffraction data and the kernel.   */
         ...
         *  Note, we set fft_out to point to the same data as fft_ker, so     *
         *  this computation reads fft_out = fft_ker * fft_in, or             *
         *  fft_ker *= fft_in.                                                */
        for (n = 0; n < arr_size; ++n)
            tmpl_CDouble_MultiplyBy(&fft_out[n], &fft_in[n]);

        /*  The original convulation can be obtained by using the inverse FFT *
         *  on the pointwise product of the two FFTs.                         */
