...
t1 = time.time()
data = '../v4/savefiles/data.txt'
df = pd.read_csv(data)
#Restore Parameters
res_km           = 1.                             #Requested Resolution (km)
rrange           = np.array([87410,87610])        #Range to compute inversion
#Restore Variables
rho_km_vals      = np.array(df.rho_km_vals)       #Ring intercept point radius
phase_rad_vals   = np.array(df.phase_rad_vals)    #Uncorrected phase, radians
raw_power_vals   = np.array(df.p_norm_vals)       #Uncorrected power
phi_rad_vals     = np.array(df.phi_rad_vals)      #Ring-azimuth angle, radians
b_rad_vals       = np.array(df.b_rad_vals)        #Ring-opening angle, radians
f_sky_hz_vals    = np.array(df.f_sky_hz_vals)     #Sky frequency values, in Hz
d_km_vals        = np.array(df.d_km_vals)         #Spacecraft-rip distance, km
rho_dot_kms_vals = np.array(df.rho_dot_kms_vals)  #drho/dt, in km/s
dx_km            = rho_km_vals[1]-rho_km_vals[0]  #Distance between points,km
