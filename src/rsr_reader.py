...
class RSRReader(object):
    ...
    def get_IQ(self,spm_range=None, decimate_16khz_to_1khz=None, TEST=False):
        ...
        if ((type(decimate_16khz_to_1khz) != bool)
                & (decimate_16khz_to_1khz is not None)):
        ...
        # Set up unpacking of full RSR. Default is full data set
        try:
            if spm_range is None:
                spm_range = [min(spm_vals), max(spm_vals)]
            self.__set_sfdu_unpack(spm_range)
        except TypeError as err:
            print('spm_range input must be 2-element list of ints or floats: '
                + '{}'.format(err))
            sys.exit()
        if len(spm_range) > 2:
            print('WARNING (RSRReader.get_IQ): spm_range input should be 2 '
                + 'elements long. Using first 2 elements of list')
...
if __name__ == '__main__':
    ...
    #spm_range = ['Bob', 'Sam', 'David']
    spm_range = [27000, 30000, 1e6]
    spm_vals, IQ_m = rsr_inst.get_IQ(spm_range=spm_range, decimate_16khz_to_1khz='a')
